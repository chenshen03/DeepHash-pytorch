...
def train(config):
    ...
    ## train   
    len_train1 = len(dset_loaders["train_set"]) - 1
    ...
    for i in range(config["num_iterations"]):
        ...
        optimizer.zero_grad()
        if i % len_train1 == 0:
            iter1 = iter(dset_loaders["train_set"])
        inputs, labels = iter1.next()
        if use_gpu:
        ...
        similarity_loss = loss.pairwise_loss_exam(outputs, labels, \
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prefix', type=str, default='ResNet50_hashnet', help="save path prefix")
    # parser.add_argument('--net', type=str, default='AlexNet', help="base network type")
    # parser.add_argument('--prefix', type=str, default='AlexNet_hashnet', help="save path prefix")
